...
import { Home, Calendar, ClipboardList, Megaphone, LogOut, Settings } from 'lucide-react';
...
export default function StaffLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { label: 'Home', path: '/my', icon: <Home size={22} strokeWidth={1.5} /> },
    { label: 'Attendance', path: '/my/attendance', icon: <Calendar size={22} strokeWidth={1.5} /> },
    { label: 'Leave', path: '/my/leave', icon: <ClipboardList size={22} strokeWidth={1.5} /> },
    { label: 'Updates', path: '/my/announcements', icon: <Megaphone size={22} strokeWidth={1.5} /> },
  ];
    ...
    <div className="min-h-screen bg-[#F8F8F8] flex flex-col relative app-frame select-none font-sans">
      ...
      <header className="bg-white text-[#1A1A1A] sticky top-0 z-[1000] px-4 py-3 flex items-center justify-between min-h-[56px] border-b border-[#E8E8E8] shadow-[0_1px_3px_rgba(0,0,0,0.06)] pt-[calc(12px+env(safe-area-inset-top,0px))]">
        <div className="flex items-center space-x-2">
          <div className="font-[900] text-[20px] tracking-tight flex items-center cursor-pointer" onClick={() => router.push('/my')}>
        ...
        <div className="flex items-center space-x-3">
          ...
          >
            <Settings size={18} strokeWidth={1.5} style={{ color: 'currentColor' }} />
          </button>
